...
def findARestaurant(mealType, location):
    ...
    if result:
        ...
        if result['response']['photos']['items']:
            ...
            imageURL = firstpic['prefix'] + "300x300" + firstpic['suffix']
        else:
            # 6.  if no image available, insert default image url
            imageURL = "http://pixabay.com/get/8926af5eb597ca51ca4c/1433440765/cheeseburger-34314_1280.png?direct"
        # 7.  return a dictionary containing the restaurant name, address, and image url
        restaurantDict = {'name': restaurant_name, 'address': restaurant_address, 'image': imageURL}
        print "Restaurant Name: %s" % restaurantDict['name']
        print "Restaurant Address: %s" % restaurantDict['address']
        print "Image: %s \n" % restaurantDict['image']
